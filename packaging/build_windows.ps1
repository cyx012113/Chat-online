...
$DistRoot = Get-SafeWorkspacePath (Join-Path $ProjectRoot "dist")
$PythonDist = Get-SafeWorkspacePath (Join-Path $DistRoot "python")
...
$ReleaseDir = Get-SafeWorkspacePath (Join-Path $DistRoot $ReleaseName)
$ZipPath = Get-SafeWorkspacePath (Join-Path $DistRoot "$ReleaseName.zip")
$ZipHashPath = Get-SafeWorkspacePath (Join-Path $DistRoot "$ReleaseName.zip.sha256")
...
try {
    ...
    & $Python -m pip install -e ".[dev]" -r ".\packaging\requirements-build.txt"
    ...
    Copy-Item -LiteralPath ".\README.md", ".\LICENCE", ".\packaging\THIRD_PARTY_NOTICES.md" -Destination $ReleaseDir
